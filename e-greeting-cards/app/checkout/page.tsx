...
export default function CheckoutPage() {
  ...
  // Initialise Paddle.js once the script loads
    ...
    window.Paddle.Initialize({
      ...
      eventCallback: (event: any) => {
        if (event.name === 'checkout.completed') {
          // Paddle overlay completed — redirect to confirmation
          const cardId = sessionStorage.getItem('pendingCardId');
          sessionStorage.removeItem('pendingCardId');
          if (cardId) {
            router.push(`/confirmation/${cardId}`);
          }
        }
      },
    });
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      const { cardId, transactionId } = await response.json();

      // Store cardId so the eventCallback can redirect after payment
      sessionStorage.setItem('pendingCardId', cardId);

      // Open Paddle overlay checkout
      if (!window.Paddle) {
        throw new Error('Paddle.js has not loaded yet. Please try again.');
      }

      window.Paddle.Checkout.open({
        transactionId,
        settings: {
          successUrl: `${window.location.origin}/confirmation/${cardId}`,
        },
      });
            ...
            >
              <div>
                ...
                >
                  <p className="text-sm text-gray-600 mb-3">
                    To ({cardDraft.recipients.length} recipient{cardDraft.recipients.length !== 1 ? 's' : ''})
                  </p>
              ...
              >
                ...
                <div className="bg-gray-50 rounded-lg p-4 text-sm text-gray-600 space-y-2">
                  <p><strong>Headline:</strong> {cardDraft.customText.headline || '(Not set)'}</p>
                  <p><strong>Message:</strong> {cardDraft.customText.body || '(Not set)'}</p>
                  <p><strong>Signature:</strong> {cardDraft.customText.signature || '(Not set)'}</p>
                </div>
              </motion.div>
